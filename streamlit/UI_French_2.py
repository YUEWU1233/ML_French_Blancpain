...
model_path = r"D:/Lausanne_Life/MA2/ML/French/results/三个数据集/SVC().pkl"#put in the file location here
model = joblib.load(model_path)
scaler = joblib.load("D:/Lausanne_Life/MA2/ML/French/results/两个数据集/scaler.pkl")#put in the file location here
def preprocess(text):
    """preprocess the text"""
...
def evaluate_difficulty(text):
    ...
    difficulty_labels = pd.Series(predictions).map({
        0: 'A1', 1: 'A2', 2: 'B1', 3: 'B2', 4: 'C1', 5: 'C2'
    })

    # return the difficulty label
    return difficulty_labels.iloc[0]
